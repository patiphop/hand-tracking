...
class CanvasBasedPreprocessing {
  ...
  async Preprocess(
    ...
    if (!this.pad_) {
      ...
      const ctx: any = trg.getContext('2d');
      ...
      if (width > height) {
        ...
        ctx.clearRect(
          0, 
          this.resizeHeight_ - this.resizeHeight_ * boxPad / 2,
          this.resizeWidth_, 
          boxPad / 2 * this.resizeHeight_
        );
      } else if (width < height) {
        const boxPad = 1 - (width / height);
        ctx.clearRect(0, 0, boxPad / 2 * this.resizeWidth_, this.resizeHeight_);
        ctx.clearRect(
          this.resizeWidth_ - this.resizeWidth_ * boxPad / 2, 
          0, 
          boxPad / 2 * this.resizeWidth_, 
          this.resizeHeight_
        );
      }
      return res;
